...
impl Parser {
    ...
    fn lex(&mut self, r: &mut dyn CharReader) -> Result<Token, Error> {
        ...
        fn consume_string_starting_with_indicator(r: &mut dyn CharReader, t: Terminal) -> Result<Token, Error> {
            if let Some(nc) = r.peek_char(1)? {
                if is_blank_or_break(nc) {
                    ...
                    r.skip_while(&mut is_string)?;
                    let p2 = r.position();
                    return Ok(Token::new(Terminal::String, p1, p2));
                }
            } else {
                return consume(r, 1, t);
            }
        ...
        match r.peek_char(0)? {
            ...
            Some('"') => {
                return consume_surrounded_string(r, '"', Terminal::String, &mut is_string_with_escapes);
            },
            Some('\'') => {
                return consume_surrounded_string(r, '\'', Terminal::String, &mut is_string_with_apostrophes);
            },
